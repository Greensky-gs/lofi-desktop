...
const getPlaylists = () => {
    const keys = JSON.parse((localStorage.getItem('playlist_keys') ?? '{}')) as Record<string, string>;
...
const hardPlaylist = (playlist: playlist): hardPlaylistType => {
    return {
        ...playlist,
        stations: playlist.stations.map(x => window.stations.find(y => y.url === `https://www.youtube.com/watch?v=${x}`))
    }
}
