...
const shuffle = <T = any>(array: T[]): T[] => {
    return array.map(x => ([ x, Math.random() ])).sort((a, b) => (b[1] as number) - (a[1] as number)).map(x => x[0] as T)
}
const callback = (stations: hardStation[]) => {
    const container = document.getElementById('container')

    shuffle(stations.filter(x => x.type === 'playlist')).forEach((station) => {
        ...
        const maxLine = 75
        shadower.style.width = `${Math.min(maxLine, title.length) * 7 + 30}px`
        shadower.style.height = `${Math.ceil(title.length / maxLine) * 45}px`
